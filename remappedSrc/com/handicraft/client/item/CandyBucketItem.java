...
public class CandyBucketItem extends Item {
    public CandyBucketItem() {
        ...
        if (FabricLoader.getInstance().getEnvironmentType() == EnvType.CLIENT) {
            FabricModelPredicateProviderRegistry.register(this,new Identifier("filled"),(stack, world, entity) -> {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        Inventories.fromTag(stack.getOrCreateTag(),items);
        int slot = hand == Hand.OFF_HAND ? -1 : user.inventory.selectedSlot;
        if (!world.isClient) {
    ...
    public static void setItems(ItemStack stack, DefaultedList<ItemStack> items) {
        CompoundTag tag = Inventories.toTag(new CompoundTag(),items);
