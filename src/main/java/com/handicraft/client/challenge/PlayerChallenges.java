...
public class PlayerChallenges {
    ...
    public static List<ChallengeInstance> readFrom(NbtList tag) {
        List<ChallengeInstance> challenges = new ArrayList<>();
        for (NbtElement t : tag) {
            if (t instanceof NbtCompound) {
                ChallengeInstance in = ChallengeInstance.fromNBT((NbtCompound)t,i->ChallengesManager.get(CommonMod.SERVER.get().getOverworld()).get(i));
