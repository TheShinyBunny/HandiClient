...
public class EnderChestScreen extends HandledScreen<EnderChestScreenHandler> {
    ...
    protected void init() {
        super.init();
        addButton = addButton(new HoldableImageButton(x + backgroundWidth + 16,y + backgroundHeight / 2 - 40,20,20,0,0,20,ICONS,20,20,40,60,b->{
            int points = handler.addXP();
            PacketByteBuf buf = new PacketByteBuf(Unpooled.buffer());
            buf.writeVarInt(points);
            ClientSidePacketRegistry.INSTANCE.sendToServer(CommonMod.CHANGE_STORED_XP,buf);
        },STORE_TOOLTIP,50));
        takeButton = addButton(new HoldableImageButton(x + backgroundWidth + 16,y + backgroundHeight / 2,20,20,20,0,20,ICONS,20,20,40,60, b->{
