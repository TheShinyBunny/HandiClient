...
public class CashRegisterOwnerScreen extends HandledScreen<CashRegisterOwnerHandler> {
    ...
    protected void init() {
        super.init();
        addDrawableChild(passwordField = new TextFieldWidget(textRenderer,x + backgroundWidth / 2 - 70,y - 22,100,20,new TranslatableText("container.cash_register.password")));
        passwordField.setChangedListener(pw->{
            ClientPlayNetworking.send(CashRegisterOwnerHandler.CHANGE_PASSWORD,new PacketByteBuf(Unpooled.buffer()).writeString(pw));
        });
        passwordField.setTextPredicate(pw->pw != null && !pw.isEmpty());
        passwordField.setText(handler.getPassword());
        addDrawableChild(costField = new TextFieldWidget(textRenderer,x + 8,y + 53,48,18,new TranslatableText("container.cash_register.cost")));
        ...
        ButtonWidget claim = addDrawableChild(new ButtonWidget(x + 10,y + 105,46,18,new TranslatableText("container.cash_register.claim"),b->{
        ...
        this.remove = addDrawableChild(new ButtonWidget(x + backgroundWidth + 2,y,70,20,new TranslatableText("container.cash_register.remove"),b->{
    ...
    protected void drawForeground(MatrixStack matrices, int mouseX, int mouseY) {
        ...
        itemRenderer.renderGuiItemIcon(new ItemStack(Items.EMERALD),32 - (w / 2) + textW + 2,87);
    }
