...
import com.handicraft.client.rewards.EmoteReward;
...
public class PlayerCollectibles {
    ...
    public <T extends Collectible> List<T> getOwned(CollectibleType<T> type) {
        return owned.stream().filter(c->c.getType() == type).map(c->(T)c).collect(Collectors.toList());
    }

    public <T extends Collectible> T getSelected(CollectibleType<T> type) {
        return (T) selected.get(type);
    }
    ...
    public static PlayerCollectibles load(MinecraftServer server, UUID uuid) {
        ...
        try {
            ...
            CompoundTag sel = tag.getCompound("selectedCollectibles");
    ...
    public void writePacket(PacketByteBuf buf) {
        ...
        HandiUtils.writeMap(buf,selected,(b,t)->b.writeString(t.getId()),(b,c)->b.writeVarInt(Collectibles.REGISTRY.getRawId(c)));
